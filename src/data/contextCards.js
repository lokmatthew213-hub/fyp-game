...
// Each card defines the data context used for
// judging Percent Battle submissions.
// ─────────────────────────────────────────────
...
/**
 * Draw a random context card from the deck.
 * Optionally exclude a specific card id to avoid repeating.
 */
export const drawContextCard = (excludeId = null) => {
...
/**
 * Build a context data object compatible with aiJudge.js
 * from a context card.
 */
export const buildContextData = (card) => {
  const data = { base: card.base };
  card.segments.forEach(seg => {
    data[seg.key] = seg.value;
  });
  return data;
};
