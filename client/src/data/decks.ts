...
const createCard = (type: CardType, value: string, count: number, label: string = value): Card[] => {
...
export const generateNDeck = (): Card[] => {
  const deck = [
...
export const generateWDeck = (): Card[] => {
  const deck = [
    ...createCard(CARD_TYPES.WORD, '全部', 10),
    ...createCard(CARD_TYPES.WORD, '佔/是', 10),
